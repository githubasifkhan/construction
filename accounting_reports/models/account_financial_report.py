...
class ReportAccountFinancialReport(models.Model):
    ...
    def _get_filter_journals(self):
        if self == self.env.ref('accounting_reports.account_financial_report_cashsummary0'):
    ...
    def _create_action_and_menu(self, parent_id):
        ...
        module = self._context.get('install_module', 'accounting_reports')
...
class AccountFinancialReportLine(models.Model):
    ...
    def _get_with_statement(self, financial_report):
        ...
        if financial_report == self.env.ref('accounting_reports.account_financial_report_cashsummary0'):
    ...
    def _get_lines(self, financial_report, currency_table, options, linesDicts):
        ...
        for line in self:
            ...
            vals = {
                ...
                'name': line.name,
                'level': line.level,
                'class': 'o_accounting_reports_totals_below_sections' if self.env.user.company_id.totals_below_sections else '',
            ...
            if line.id in options.get('unfolded_lines', []) or line.show_domain == 'always':
                ...
                if domain_ids and self.env.user.company_id.totals_below_sections:
                    lines.append({
                        ...
                        'name': _('Total') + ' ' + line.name,
                        'level': line.level,
                        'class': 'o_accounting_reports_domain_total',
                        'parent_id': line.id,
