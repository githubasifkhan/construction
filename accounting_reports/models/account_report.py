...
class AccountReport(models.AbstractModel):
    ...
    def _get_templates(self):
        return {
                'main_template': 'accounting_reports.main_template',
                'main_table_header_template': 'accounting_reports.main_table_header',
                'line_template': 'accounting_reports.line_template',
                'footnotes_template': 'accounting_reports.footnotes_template',
                'search_template': 'accounting_reports.search_template',
        }
    ...
    def open_general_ledger(self, options, params=None):
        ...
        ctx = self.env.context.copy()
        ctx.pop('id', '')
        action = self.env.ref('accounting_reports.action_account_report_general_ledger').read()[0]
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        ...
        * if line_id is set, the template used will be the line_template
        ...
        render_template = templates.get('main_template', 'accounting_reports.main_template')
        if line_id is not None:
            render_template = templates.get('line_template', 'accounting_reports.line_template')
    ...
    def get_html_footnotes(self, footnotes):
        template = self._get_templates().get('footnotes_template', 'accounting_reports.footnotes_template')
    ...
    def _replace_class(self):
        ...
        """
        return {b'o_accounting_reports_no_print': b'', b'table-responsive': b'', b'<a': b'<span', b'</a>': b'</span>'}
    ...
    def get_pdf(self, options, minimal_layout=True):
        ...
        body = self.env['ir.ui.view'].render_template(
            "accounting_reports.print_template",
            values=dict(rcontext),
        )
        body_html = self.with_context(print_mode=True).get_html(options)

        body = body.replace(b'<body class="o_accounting_reports_body_print">', b'<body class="o_accounting_reports_body_print">' + body_html)
        if minimal_layout:
            header = ''
