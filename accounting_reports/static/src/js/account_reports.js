odoo.define('accounting_reports.account_report', function (require) {
'use strict';
...
var accountReportsWidget = AbstractAction.extend(ControlPanelMixin, {
    ...
    events: {
        'input .o_accounting_reports_filter_input': 'filter_accounts',
        'click .o_accounting_reports_summary': 'edit_summary',
        'click .js_account_report_save_summary': 'save_summary',
        'click .o_accounting_reports_footnote_icons': 'delete_footnote',
        'click .js_accounting_reports_add_footnote': 'add_edit_footnote',
        'click .js_account_report_foldable': 'fold_unfold',
        'click [action]': 'trigger_action',
        'click .o_accounting_reports_load_more span': 'load_more',
    },
    ...
    render_template: function() {
        ...
        this.$el.find('.o_accounting_reports_summary_edit').hide();
    ...
    filter_accounts: function(e) {
        ...
        this.filterOn = false;
        this.$('.o_accounting_reports_level2').each(function(index, el) {
            ...
            $accountReportLineFoldable.toggleClass('o_accounting_reports_filtered_lines', !queryFound);
            $childs.toggleClass('o_accounting_reports_filtered_lines', !queryFound);
        ...
        if (this.filterOn) {
            this.$('.o_accounting_reports_level1.total').hide();
        }
        else {
            this.$('.o_accounting_reports_level1.total').show();
        }
    ...
    render_searchview_buttons: function() {
        ...
        var $datetimepickers = this.$searchview_buttons.find('.js_accounting_reports_datetimepicker');
        ...
        _.each(this.$searchview_buttons.find('.js_accounting_reports_one_choice_filter'), function(k) {
        ...
        this.$searchview_buttons.find('.js_accounting_reports_one_choice_filter').click(function (event) {
        ...
        // analytic filter
        this.$searchview_buttons.find('.js_accounting_reports_analytic_auto_complete').select2();
        ...
        this.$searchview_buttons.find('.js_accounting_reports_analytic_auto_complete').on('change', function(){
    ...
    edit_summary: function(e) {
        var $textarea = $(e.target).parents('.o_accounting_reports_body').find('textarea[name="summary"]');
        var height = Math.max($(e.target).parents('.o_accounting_reports_body').find('.o_account_report_summary').height(), 100); // Compute the height that will be needed
        ...
        $textarea.val(text);
        $(e.target).parents('.o_accounting_reports_body').find('.o_accounting_reports_summary_edit').show();
        $(e.target).parents('.o_accounting_reports_body').find('.o_accounting_reports_summary').hide();
        $(e.target).parents('.o_accounting_reports_body').find('textarea[name="summary"]').focus();
    },
    save_summary: function(e) {
        ...
        return this._rpc({
            ...
            .then(function(result){
                self.$el.find('.o_accounting_reports_summary_edit').hide();
                self.$el.find('.o_accounting_reports_summary').show();
                if (!text) {
                ...
                return $(e.target).parent().siblings('.o_accounting_reports_summary').find('> .o_account_report_summary').html($content);
            });
    },
    render_footnotes: function() {
        ...
        _.each($dom_footnotes, function(el) {
            if ($(el).parents('.o_accounting_reports_filtered_lines').length > 0) {
                return;
            }
            var line_id = $(el).data('id');
            var footnote = _.filter(self.footnotes, function(footnote) {return ''+footnote.line === ''+line_id;});
            if (footnote.length !== 0) {
                $(el).html('<sup><b class="o_accounting_reports_footnote_sup"><a href="#footnote'+number+'">'+number+'</a></b></sup>');
    ...
    add_edit_footnote: function(e) {
        ...
        var save = function() {
            var footnote_text = $('.js_accounting_reports_footnote_note').val().replace(/[ \t]+/g, ' ');
            if (!footnote_text && existing_footnote.length === 0) {return;}
            if (existing_footnote.length !== 0) {
                if (!footnote_text) {
                    return self.$el.find('.footnote[data-id="'+existing_footnote[0].id+'"] .o_accounting_reports_footnote_icons').click();
                }
    ...
    fold_unfold: function(e) {
        var self = this;
        if ($(e.target).hasClass('caret') || $(e.target).parents('.o_accounting_reports_footnote_sup').length > 0){return;}
        e.stopPropagation();
        e.preventDefault();
    ...
    load_more: function (ev) {
        ...
        this._rpc({
            ...
            .then(function (result){
                var $tr = $line.parents('.o_accounting_reports_load_more');
                $tr.after(result);
                $tr.remove();
                self._add_line_classes();
            });
